(function () {
    ...
    var neutralZoomLevel = 5,
        scalePerZoomLevel = 1.5,
        zoomLevels = 10;



    angular.module('ipg.imageViewer', ['monospaced.mousewheel'])
        .directive('ipgImageViewer', ['$document', 'PanZoomService',
            ...
            controller: ['$scope', '$element',
                function ($scope, $element) {
                    ...
                    var getZoomLevel = function (cssScale) {
                        return Math.log(cssScale) / Math.log(scalePerZoomLevel) + neutralZoomLevel;
                    };
                    ...
                    zoomLevels = zoomLevels !== undefined ? zoomLevels : 5;
                    neutralZoomLevel = neutralZoomLevel !== undefined ? neutralZoomLevel : 2;
                    ...
                    scalePerZoomLevel = scalePerZoomLevel !== undefined ? scalePerZoomLevel : 2;
                    ...
                    $scope.config.initialZoomLevel =
                        $scope.config.initialZoomLevel !== undefined ? $scope.config.initialZoomLevel : neutralZoomLevel;
                    ...
                    if ($scope.config.keepInBounds && neutralZoomLevel !== 0) {
                        console.warn('You have set keepInBounds to true and neutralZoomLevel to ' + neutralZoomLevel +
                                     '. Be aware that the zoom level cannot below ' + neutralZoomLevel);
                    }
                    ...
                    var syncModelToDOM = function () {
                        if ($scope.zoomAnimation) {
                            ...
                            $scope.model.zoomLevel = $scope.base.zoomLevel;
                            $scope.model.pan = $scope.model.pan || {};
                    ...
                    var changeZoomLevel = function (newZoomLevel, clickPoint, duration) {
                        ...
                        var minimumAllowedZoomLevel = $scope.config.keepInBounds ? neutralZoomLevel : 0;
                        newZoomLevel = Math.max(minimumAllowedZoomLevel, newZoomLevel);
                        newZoomLevel = Math.min(zoomLevels - 1, newZoomLevel);
